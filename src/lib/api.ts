 ...
 * (requires the server to expose it via Access-Control-Expose-Headers).
 */
...
/** Applies the edited document for a cached job and returns the updated PDF. */
...
/**
 * Fallback render: POST the edited document as a JSON file to produce the
 * PDF without a job id (used when the X-Job-Id header is not readable).
 */
